...
async function verifyRiseStop (account, data, trade, updateTradesOn) {
  let stopPrice
  if (trade[TRADES_ON.SIDE] === SIDE.BUY) {
    if (trade[TRADES_ON.RISE_STOP_PRICE] < data.k.c) {
      ...
      await changeStopLoss(account, stopPrice, trade, updateTradesOn, TRADES_ON.RISE_STOP_CREATED)
    } else if (!trade[TRADES_ON.BREAKEVEN_CREATED] && trade[TRADES_ON.BREAKEVEN_PRICE] < data.k.c) {
      console.log('breakevenFunction')

      await changeStopLoss(account, trade[TRADES_ON.ENTRY_PRICE], trade, updateTradesOn, TRADES_ON.BREAKEVEN_CREATED)
    }
