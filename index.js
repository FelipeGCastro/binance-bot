...
async function execute (account) {
    ...
    if (!hasTradeOn &&
      ...
      if (valid && valid.symbol === candlesObj.symbol) {
        const ordered = await newOrder.handleNewOrder({
          ...
          maxEntryValue: ACCOUNT.maxEntryValue,
          symbol,
          account
        })
        if (ordered) {
          setAccountData(ACCOUNT_PROP.LIMIT_REACHED, (tradesOn.length + 1) >= ACCOUNT.limitOrdersSameTime)
          setTradesOn({
          ...
          telegram.sendMessage(`Entrou: ${symbol}PERP, Side: ${valid.side}, Strategy: ${ACCOUNT.strategy}, account: ${account}`)
  ...
  async function setWsListen (listenKey) {
    const wsListenKey = ws.listenKey(listenKey, async (data) => {
      if (data.e === 'listenKeyExpired' && ACCOUNT.listenKeyIsOn) {
  ...
  function handleGetStopAndTarget (account, entryPrice, stopPrice, side) {
    ...
    if (ACCOUNT.strategy === STRATEGIES.HIDDEN_DIVERGENCE) {
      return getStopAndTargetPrice(stopPrice, entryPrice)
    } else if (ACCOUNT.strategy === STRATEGIES.SHARK) {
      return false
    } else return false
  }
